...
export default function InvestmentsDashboard() {
  ...
  const payoutSchema = useMemo(() => {
    ...
    return list
      .map((x) => ({ ...x, share: grandTotal ? (x.total / grandTotal) * 100 : 0 }))
      .slice(0, 6)
  }, [filteredRows])
  ...
  const topAffiliate = useMemo(
    () => (topAffiliates.length ? topAffiliates[0] : null),
    [topAffiliates]
  )
          ...
          <div className="card card-global">
            ...
            <div className="kpi-grid" style={{ gridTemplateColumns: '1fr' }}>
              ...
              <KpiCard
                label="Top payout"
                value={
                  topPayout ? `${topPayout.type} (${formatPercentRounded(topPayout.share)})` : '—'
                }
                    ...
                    return (
                      <tr key={t.type}>
                        ...
                        <td
                          ...
                          className="num"
                          title={`${t.count} transazioni`}
                        >
                          {formatNumberShort(t.count)}
                        </td>
                            ...
                            >
                              <div
                                style={{
                                  ...
                                  top: 0,
                                  bottom: 0,
                                  width: `${Math.min(100, Math.max(0, t.share))}%`,
                                  background: '#34d399',
                                  borderRadius: 999,
                                }}
                              />
                  ...
                  {payoutSchema.length > 0 && (
                    <tr style={{ borderTop: '1px solid rgba(148,163,184,0.15)' }}>
                      ...
                      <td
                        style={{ textAlign: 'right', color: '#fbbf24' }}
                        className="num"
                        title={formatEuroFull(payoutSchemaTotals.total)}
                      >
                        {formatEuro(payoutSchemaTotals.total)}
                      </td>
                      <td
                        style={{ textAlign: 'right' }}
                        className="num"
                        title={`${payoutSchemaTotals.count} transazioni`}
                      >
                        {formatNumberShort(payoutSchemaTotals.count)}
                      </td>
                      <td style={{ textAlign: 'right', color: '#34d399' }} className="num">
                        {formatPercentRounded(payoutSchemaTotals.share)}
                      </td>
                    </tr>
                  )}
                </tbody>
                  ...
                  {monthlyAggregates.map((m) => (
                    <tr key={m.monthKey}>
                      ...
                      <td
                        style={{ textAlign: 'right', color: '#fbbf24' }}
                        className="num"
                        title={formatEuroFull(m.total)}
                      >
                        {formatEuro(m.total)}
                      </td>
                      <td style={{ textAlign: 'right' }} className="num">
                        {formatNumberShort(m.count)}
                      </td>
                      <td
                        style={{ textAlign: 'right' }}
                        className="num"
                        title={formatEuroFull(m.count ? m.total / m.count : 0)}
                      >
                        {formatEuro(m.count ? m.total / m.count : 0)}
                      </td>
                    </tr>
                  ))}
