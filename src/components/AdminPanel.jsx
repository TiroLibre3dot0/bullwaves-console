    ...
    <div className="grid-global" style={{ marginTop: 12 }}>
      ...
      <div className="card">
        ...
        {onlineUsers.length === 0 ? (
          ...
          <div className="stack">
            {onlineUsers.map((u) => (
              <div key={u.email} className="list-item" style={{ justifyContent: 'flex-start', gap: 12 }}>
                <span className="status-dot online" aria-hidden="true" />
                <div>
                  <div className="user-name">{u.name || u.email}</div>
                  <div className="text-muted small">{u.role || '—'} · {u.email}</div>
                  <div className="text-muted small">Last seen: {formatTime(u.lastSeen)} {u.lastSection ? `• ${sectionLabels[u.lastSection] || u.lastSection}` : ''}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="card">
        <h2>Most visited sections</h2>
        {sectionStats.length === 0 ? (
          <div className="text-muted">No navigation events yet.</div>
        ) : (
          <div className="stack">
            {sectionStats.map((s) => (
              <div key={s.sectionId} className="list-item" style={{ gap: 10 }}>
