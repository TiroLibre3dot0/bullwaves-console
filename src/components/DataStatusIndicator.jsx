  ...
  const content = (
    ...
    >
      ...
      {showPopup && onClosePopup && (
        <button onClick={onClosePopup} className="ml-2 text-gray-500 hover:text-gray-700">
          ×
        </button>
      )}
    </div>
      ...
      <>
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 text-center">
            <div
              className={`w-16 h-16 mx-auto mb-4 rounded-full bg-gradient-to-br ${getPopupBg()} flex items-center justify-center text-3xl`}
            >
              {getIcon()}
            </div>
            ...
            <button
              ...
              className="px-6 py-2 bg-gray-700 text-white rounded-full hover:bg-gray-700 transition-colors"
            >
              Chiudi
            </button>
          </div>
