...
export default function GlobalDashboard() {
      ...
      try {
        ...
        const parsed = parseCsv(text).map((r) => {
          ...
          return {
            ...
            uniqueImpressions: cleanNumber(
              pick(r, ['Unique Impressions', 'unique_impressions', 'uniqueimpressions'])
            ),
            ctr: cleanPercent(pick(r, ['CTR', 'ctr'])),
            uniqueVisitors: cleanNumber(
              pick(r, ['Unique Visitors', 'unique_visitors', 'uniquevisitors'])
            ),
            ...
            conversionRate: cleanPercent(
              pick(r, ['Conversion Rate', 'conversion_rate', 'conversionrate'])
            ),
            ...
            firstDeposits: cleanNumber(
              pick(r, ['First Deposits', 'first_deposits', 'firstdeposits'])
            ),
            ...
            cpaCommission: cleanNumber(
              pick(r, ['CPA Commission', 'cpa_commission', 'cpaCommission', 'CPA'])
            ),
            cplCommission: cleanNumber(
              pick(r, ['CPL Commission', 'cpl_commission', 'cplCommission', 'CPL'])
            ),
            revShareCommission: cleanNumber(
              pick(r, [
                'RevShare Commission',
                'revshare_commission',
                'revShareCommission',
            ...
            subCommission: cleanNumber(
              pick(r, ['Sub Commission', 'sub_commission', 'subCommission', 'subaffiliate'])
            ),
            otherCommission: cleanNumber(
              pick(r, ['Other Commission', 'other_commission', 'otherCommission', 'other'])
            ),
          }
        })
        setMediaRows(parsed)
  ...
  const monthOptions = useMemo(() => {
    const byKey = new Map()
    const add = (monthIndex, monthLabel) => {
      if (monthIndex === undefined || monthIndex === null) return
      const idx = Number(monthIndex)
      if (!Number.isFinite(idx)) return
      const key = String(idx)
      if (!byKey.has(key)) byKey.set(key, monthLabel || `Month ${idx + 1}`)
    }

    if (selectedYear !== 'all') {
      const year = Number(selectedYear)
      ;(periodOptions.availableMonths[year] || []).forEach((m) => add(m.value, m.label))
    } else {
      periodOptions.availableYears.forEach((year) => {
        ;(periodOptions.availableMonths[year] || []).forEach((m) => add(m.value, m.label))
      })
    }
  ...
  const breakEven = useMemo(() => {
    ...
    const firstActive = cumulative.findIndex(
      (v, idx) => idx === 0 || months[idx].pl || months[idx].payments
    )
    const beIndex = cumulative.findIndex(
    ...
    <div className="w-full space-y-3">
        ...
        <div className="card card-global">
          ...
          <div className="kpi-grid">
            <div className="kpi" title="Visitors = somma visitors nel periodo filtrato">
              <span>Visitors</span>
              <strong title={formatNumberFull(totals.visitors)}>
                {formatNumberShort(totals.visitors)}
              </strong>
            </div>
            <div className="kpi" title="Registrations = somma registrazioni nel periodo filtrato">
              <span>Registrations</span>
              <strong title={formatNumberFull(totals.registrations)}>
                {formatNumberShort(totals.registrations)}
              </strong>
            </div>
            ...
            <div className="kpi" title="Impressions = somma impressions">
              ...
              <strong title={formatNumberFull(totals.impressions)}>
                {formatNumberShort(totals.impressions)}
              </strong>
            </div>
          </div>
        </div>
        ...
        <div className="card card-global">
          ...
          <div className="kpi-grid">
            <div className="kpi" title="Net deposits = somma net deposits filtrati">
              ...
              <strong
                title={formatEuroFull(totals.netDeposits)}
                style={{ color: (totals.netDeposits || 0) >= 0 ? '#34d399' : '#f87171' }}
              >
                {formatEuro(totals.netDeposits)}
              </strong>
            </div>
            <div className="kpi" title="P&L = somma PL (trading) filtrati">
              <span>P&L</span>
              <strong
                title={formatEuroFull(totals.pl)}
                style={{ color: (totals.pl || 0) >= 0 ? '#34d399' : '#f87171' }}
              >
                {formatEuro(totals.pl)}
              </strong>
            </div>
            <div className="kpi" title="Payments pagati (Payments Report)">
              <span>Payments</span>
              <strong title={formatEuroFull(totals.paymentsTotal)} style={{ color: '#fbbf24' }}>
                {formatEuro(totals.paymentsTotal)}
              </strong>
            </div>
            <div className="kpi" title="Profit = PL - Payments">
              <span>Profit</span>
              <strong
                title={formatEuroFull(totals.profit)}
                style={{ color: (totals.profit || 0) >= 0 ? '#34d399' : '#f87171' }}
              >
            ...
            <div className="kpi" title="ROI medio semplice sulle righe filtrate">
              <span>ROI avg</span>
              <strong style={{ color: (totals.roiAvg || 0) >= 0 ? '#34d399' : '#f87171' }}>
                {formatPercentDisplay(totals.roiAvg)}
              </strong>
            </div>
            <div className="kpi" title="Payout medio per FTD nel periodo">
              <span>Payout / FTD</span>
              <strong
                title={formatEuroFull(
                  totals.ftd ? totals.paymentsTotal / Math.max(totals.ftd, 1) : 0
                )}
                style={{ color: '#fbbf24' }}
              >
                {formatEuro(totals.ftd ? totals.paymentsTotal / Math.max(totals.ftd, 1) : 0)}
              </strong>
            </div>
          </div>
        </div>
      ...
      {SHOW_QUICK_INSIGHTS && (
        <div
          className="card card-global"
          style={{
            background: 'linear-gradient(120deg, rgba(34,211,238,0.08), rgba(59,130,246,0.06))',
          }}
        >
          <div
            style={{
          ...
          >
            <div>
              ...
              <p style={{ margin: 0, fontSize: 12, color: '#9fb3c8' }}>
                Pillole rapide per capire cosa succede
              </p>
            </div>
                ...
                {perMonth.map((r) => (
                  <tr key={r.monthKey}>
                    ...
                    <td
                      ...
                      className="num"
                      title={formatNumberFull(r.visitors)}
                    >
                      {formatNumberShort(r.visitors)}
                    </td>
                    <td
                      style={{ textAlign: 'right' }}
                      className="num"
                    ...
                    <td
                      style={{
                        textAlign: 'right',
                        color: (r.netDeposits || 0) >= 0 ? '#34d399' : '#f87171',
                      }}
                      className="num"
                    ...
                    <td
                      ...
                      className="num"
                      title={formatEuroFull(r.payments)}
                    >
                      {formatEuro(r.payments)}
                    </td>
                    <td
                      style={{
                        textAlign: 'right',
                    ...
                    <td
                      style={{
                        textAlign: 'right',
                        color: (r.roi || 0) >= 0 ? '#34d399' : '#f87171',
                      }}
                      className="num"
                      title={formatPercentDisplay(r.roi)}
                    >
                      {formatPercentDisplay(r.roi)}
                    </td>
                    <td
                      style={{ textAlign: 'right' }}
                      className="num"
                      title={formatPercentDisplay(r.conversion)}
                    >
                      {formatPercentDisplay(r.conversion)}
                    </td>
                  </tr>
                ))}
                {perMonth.length > 0 && (
                  <tr style={{ background: 'rgba(255,255,255,0.03)', fontWeight: 700 }}>
                    ...
                    <td
                      style={{ textAlign: 'right' }}
                      className="num"
                      title={formatNumberFull(perMonthTotals.visitors)}
                    >
                      {formatNumberShort(perMonthTotals.visitors)}
                    </td>
                    <td
                      style={{ textAlign: 'right' }}
                      className="num"
                      title={formatNumberFull(perMonthTotals.registrations)}
                    >
                      {formatNumberShort(perMonthTotals.registrations)}
                    </td>
                    <td
                      style={{ textAlign: 'right' }}
                      className="num"
                      title={formatNumberFull(perMonthTotals.ftd)}
                    >
                      {formatNumberShort(perMonthTotals.ftd)}
                    </td>
                    <td
                      style={{
                        textAlign: 'right',
                        color: (perMonthTotals.netDeposits || 0) >= 0 ? '#34d399' : '#f87171',
                      }}
                      className="num"
                      title={formatEuroFull(perMonthTotals.netDeposits)}
                    >
                      {formatEuro(perMonthTotals.netDeposits)}
                    </td>
                    <td
                      style={{
                        textAlign: 'right',
                        color: (perMonthTotals.pl || 0) >= 0 ? '#34d399' : '#f87171',
                      }}
                      className="num"
                      title={formatEuroFull(perMonthTotals.pl)}
                    >
                      {formatEuro(perMonthTotals.pl)}
                    </td>
                    <td
                      style={{ textAlign: 'right', color: '#fbbf24' }}
                      className="num"
                      title={formatEuroFull(perMonthTotals.payments)}
                    >
                      {formatEuro(perMonthTotals.payments)}
                    </td>
                    <td
                      style={{
                        textAlign: 'right',
                        color: (perMonthTotals.profit || 0) >= 0 ? '#34d399' : '#f87171',
                      }}
                      className="num"
                      title={formatEuroFull(perMonthTotals.profit)}
                    >
                      {formatEuro(perMonthTotals.profit)}
                    </td>
                    <td
                      style={{
                        textAlign: 'right',
                        color: (perMonthTotals.roi || 0) >= 0 ? '#34d399' : '#f87171',
                      }}
                      className="num"
                      title={formatPercentDisplay(perMonthTotals.roi)}
                    >
                      {formatPercentDisplay(perMonthTotals.roi)}
                    </td>
                    <td
                      style={{ textAlign: 'right' }}
                      className="num"
                      title={formatPercentDisplay(perMonthTotals.conversion)}
                    >
                      {formatPercentDisplay(perMonthTotals.conversion)}
                    </td>
                  </tr>
                )}
                {!perMonth.length && (
      ...
      <div className="card card-global">
        ...
        <p style={{ marginTop: 0, color: '#9fb3c8', fontSize: 12 }}>
          Curve cumulata di (PL - Payments) mese su mese.
        </p>
