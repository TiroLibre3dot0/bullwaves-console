...
async function checkAffiliateMappings() {
  ...
  try {
    ...
    const paymentsRes = await fetch(withReportsVersion('/Payments Report.csv'), {
      cache: 'no-store',
    })
    if (paymentsRes.ok) {
    ...
    const mediaRes = await fetch(withReportsVersion('/Media Report.csv'), {
      cache: 'no-store',
    })
    if (mediaRes.ok) {
    ...
    if (results.uniqueIds.has(exampleId)) {
      ...
      console.log(
        `   Payments: ${Array.from(results.payments[exampleId] || []).join(', ') || 'N/A'}`
      )
...
console.log(
  '💡 Script caricato! Esegui checkAffiliateMappings() nella console per avviare il controllo.'
)
