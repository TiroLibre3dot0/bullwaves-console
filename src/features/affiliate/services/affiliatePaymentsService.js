...
export async function buildAffiliatePaymentsMap(opts = {}) {
  ...
  const [regs, pays] = await Promise.all([
    loadCsv(regsPath),
    loadCsv(paymentsPath).catch(() => ({ headers: [], rows: [] })),
  ])
  ...
  for (const row of regs.rows) {
    try {
      const affiliateId = pickValue(row, regsH, [
        'affiliateid',
        'affiliate id',
        'affiliate_id',
        'affiliate',
      ...
      // NOTE: In the Bullwaves Registrations Report, `Commissions` already equals:
      // CPA + CPL + Sub Affiliate + Revshare + Other.
      // `Affiliate Commissions` is informational and may overlap with CPA, so we
      // do NOT fold it into components or totals.
      const effectiveTotal = total > 0 ? total : revshare + cpa + cpl + subaffiliate + other

      // Net Deposits
      const netDeposits = parseNumber(
        pickValue(row, regsH, ['netdeposits', 'net deposits', 'net_deposits'])
      )

      // date/month — try to pick a date field present
      const dateStr = pickValue(row, regsH, [
        'firstdepositdate',
        'firstdeposit',
        'depositdate',
        'registrationdate',
      ...
      m.contributors.push({
        id: userId || '',
        name: userName || '',
        components: { total: effectiveTotal, cpa, revshare, cpl, subaffiliate, other, netDeposits },
      })
  ...
  for (const row of pays.rows) {
    try {
      const affiliateId = pickValue(row, paysH, [
        'affiliate id',
        'affiliateid',
        'affiliate_id',
        'affiliate',
      ...
      const amountStr = pickValue(row, paysH, [
        'payment amount',
        'paymentamount',
        'payment_amount',
        'paymentamount',
