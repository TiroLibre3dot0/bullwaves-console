...
export default function ProfitAnalysisPage() {
  ...
  if (loading) {
    return (
      <FullPageLoader
        progress={35}
        subtitle={t('mediaPayments.profitAnalysis.loader.mediaReport')}
      />
    )
  }
  ...
  const profitRatioTrend = useMemo(() => {
    ...
    const values = ordered.map((m) =>
      m.netDeposits ? (m.pl / Math.max(m.netDeposits, 1)) * 100 : 0
    )
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ margin: 0, color: '#9fb3c8', fontSize: 12 }}>
            Media Report only · KPIs, affiliates, map, deposits
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <label style={{ color: '#9ca3af', fontSize: 12 }}>Year</label>
          <select
            value={selectedYear}
      ...
      >
        ...
        <KpiCard
          ...
          tone={kpis.profitRatio >= 0 ? '#34d399' : '#f87171'}
        />
      </div>
      ...
      >
        <div className="card card-global">
          ...
          >
            ...
            <span style={{ fontSize: 11, color: '#9ca3af' }}>Top 10 by PL</span>
          </div>
        ...
        <div className="card card-global">
          ...
          >
            ...
            <span style={{ fontSize: 11, color: '#9ca3af' }}>Hover to inspect</span>
          </div>
        ...
        <div className="card card-global">
          ...
          >
            ...
            <span style={{ fontSize: 11, color: '#9ca3af' }}>By month</span>
          </div>
          ...
          >
            <div>
              ...
              <p style={{ margin: 0, color: '#9ca3af', fontSize: 12 }}>
                KPI mensile (redditività dei trader)
              </p>
            </div>
            <span style={{ fontSize: 11, color: '#9ca3af' }}>
              Positivo = PL superiore ai net deposits
            </span>
          </div>
