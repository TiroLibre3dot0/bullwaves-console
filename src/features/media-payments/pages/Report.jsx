...
export default function Report() {
  ...
  const availableMonths = useMemo(() => monthOptions, [monthOptions])
  ...
  const periodLabel =
    mode === 'global'
      ...
      : mode === 'annual'
        ? `Annuale ${currentYear}`
        : selectedMonth
          ? `Mensile ${availableMonths.find((m) => m.key === selectedMonth)?.label || ''}`
          : 'Mensile'
  ...
  return (
    ...
    >
      <aside style={{ ...card, background: 'linear-gradient(135deg, #f8fbff, #eef2ff)' }}>
        ...
        >
          <div>
            <div style={{ fontSize: 12, letterSpacing: 0.2, color: '#4b5563', fontWeight: 700 }}>
              Report
            </div>
            <h2 style={{ margin: '4px 0', color: '#111827' }}>Periodo rapido</h2>
            <p style={{ ...muted, margin: 0 }}>
              Base chiara per export. Fonte: Media Report + Payments/Commissions.
            </p>
          </div>
      ...
      <aside style={{ ...card }}>
        ...
        >
          <div>
            <div style={{ fontSize: 13, color: '#4b5563', fontWeight: 600 }}>Periodo</div>
            <div style={{ fontSize: 18, fontWeight: 700, color: '#111827' }}>{periodLabel}</div>
          </div>
          <div style={{ fontSize: 12, color: '#4b5563', textAlign: 'right' }}>
            ...
            <div style={{ color: '#9ca3af', fontSize: 11 }}>
              Fonte: Media Report + Payments/Commissions
            </div>
          </div>
        </div>
        <div
        ...
        >
          ...
          <div style={{ ...card, padding: 12, boxShadow: 'none' }}>
            ...
            <div
              style={{
                fontSize: 20,
                fontWeight: 700,
                color: totals.profit >= 0 ? '#111827' : '#dc2626',
              }}
            >
          ...
          <div style={{ ...card, padding: 12, boxShadow: 'none' }}>
            ...
            <div
              style={{
                fontSize: 20,
                fontWeight: 700,
                color: totals.roi >= 0 ? '#111827' : '#dc2626',
              }}
          ...
          <div style={{ ...card, padding: 12, boxShadow: 'none' }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: '#111827' }}>
              {formatter.format(totals.registrations)} reg · {formatter.format(totals.ftd)} FTD
            </div>
      ...
      <aside style={{ ...card }}>
        ...
        >
          <h3 style={{ margin: 0, color: '#111827' }}>Top affiliati (8)</h3>
          <span style={{ fontSize: 12, ...muted }}>Ordine per profitto · Fonte media+payments</span>
        </div>
        <div style={{ overflowX: 'auto', marginTop: 10 }}>
          <table className="table" style={{ minWidth: 720, color: '#111827' }}>
            <thead>
              <tr>
                <th style={{ textAlign: 'left', color: '#4b5563', background: '#eef2ff' }}>
                  Affiliate
                </th>
                <th style={{ textAlign: 'right', color: '#4b5563', background: '#eef2ff' }}>Reg</th>
                <th style={{ textAlign: 'right', color: '#4b5563', background: '#eef2ff' }}>FTD</th>
                <th style={{ textAlign: 'right', color: '#4b5563', background: '#eef2ff' }}>CPA</th>
                <th style={{ textAlign: 'right', color: '#4b5563', background: '#eef2ff' }}>
                  ARPU
                </th>
                <th style={{ textAlign: 'right', color: '#4b5563', background: '#eef2ff' }}>ROI</th>
                <th style={{ textAlign: 'right', color: '#4b5563', background: '#eef2ff' }}>
                  Profit
                </th>
                <th style={{ textAlign: 'right', color: '#4b5563', background: '#eef2ff' }}>
            ...
            <tbody>
              {topAffiliates.length === 0 && (
                <tr>
                  <td colSpan={8} style={{ textAlign: 'center', padding: 12, color: '#9ca3af' }}>
              ...
              {topAffiliates.map((r) => (
                <tr key={`row-${r.affiliate}`} style={{ background: '#ffffff' }}>
                  ...
                  <td
                    style={{ textAlign: 'right', color: r.roi >= 0 ? '#111827' : '#dc2626' }}
                  >{`${(r.roi || 0).toFixed(1)}%`}</td>
                  <td style={{ textAlign: 'right', color: r.profit >= 0 ? '#111827' : '#dc2626' }}>
                    {formatEuroShort(r.profit)}
                  </td>
                  <td style={{ textAlign: 'right', color: '#2563eb' }}>
                    {r.breakEvenMonths !== null ? `${r.breakEvenMonths} mesi` : 'n/d'}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <aside style={{ ...card }}>
        ...
        >
          <h3 style={{ margin: 0, color: '#111827' }}>Note per export</h3>
          <span style={{ fontSize: 12, ...muted }}>
            Base layout pronta per aggiungere pulsante export
          </span>
        </div>
        <ul style={{ marginTop: 8, paddingLeft: 18, color: '#111827', lineHeight: 1.5 }}>
          <li>Layout chiaro e tabella già ordinata per profitto.</li>
          <li>
            Filtro periodo: globale (tutti i dati), annuale (anno corrente), mensile (mese
            selezionato).
          </li>
