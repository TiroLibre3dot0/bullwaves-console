...
// Loads and parses the Registrations Report CSV, normalizes fields, and provides search utilities.
import Papa from 'papaparse'

const CSV_PATH = '/Registrations Report.csv'
let _cache = null
let _headersMap = null
...
function buildHeadersMap(headers) {
  ...
  headers.forEach((h, i) => {
    ...
    map[norm] = h
    // also map a flat version without underscores to be forgiving
    map[norm.replace(/_/g, '')] = h
    map[i] = h
  })
  return map
}

function normalizeRow(row, headersMap) {
  const norm = {}
