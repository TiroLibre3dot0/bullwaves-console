...
export default function ExecutiveAnalysisEngine({ kpis, periodLabel }) {
  ...
  const recap = useMemo(() => {
    ...
    if (profit > 0)
      items.push(
        `Profit positivo (${formatEuroFull(profit)}) su ${formatEuroFull(netDeposits)} net dep.`
      )
    if (roi > 20) items.push(`ROI forte (${formatPercentRounded(roi)}) sul periodo.`)
    if (margin > 10)
      items.push(`Margine solido (${formatPercentRounded(margin)}) rispetto alla base selezionata.`)
    if (items.length === 0)
      items.push('Performance stabile, nessun segnale forte positivo o negativo.')
    return items
  ...
  const risks = useMemo(() => {
    ...
    if (volatility > Math.abs(profit) * 0.5)
      items.push('Alta volatilità mese su mese: rendimenti poco stabili.')
  ...
  const opportunities = useMemo(() => {
    const items = []
    if (recentTrend > 0)
      items.push('Trend recente in miglioramento: ampliare sui canali che performano.')
    if (conversion > 15)
      items.push(
        `Buona conversion (${formatPercentRounded(conversion)}): scalare creatività vincenti.`
      )
    if (bestMonth)
      items.push(
        `Mese migliore: ${bestMonth.monthLabel || 'n/d'} con profit ${formatEuroFull(bestMonth.profit || 0)}.`
      )
    if (items.length === 0)
      items.push('Opportunità: testare nuove offerte o geo dove il ROI è sopra la media.')
    return items
  }, [recentTrend, conversion, bestMonth])

  const outlook = useMemo(() => {
    const items = []
    if (profit > 0 && recentTrend >= 0)
      items.push('Prospettiva positiva: mantenere mix attuale e ottimizzare payout.')
    else if (profit > 0)
      items.push('Prospettiva cauta: profit ok ma trend in calo, ribilancia investimenti.')
    else
      items.push(
        'Serve inversione: concentrare budget su canali con ROI positivo e tagliare sprechi.'
      )
    if (worstMonth)
      items.push(
        `Mese critico: ${worstMonth.monthLabel || 'n/d'} (profit ${formatEuroFull(worstMonth.profit || 0)}).`
      )
    if (payments > 0)
      items.push(`Payout totale ${formatEuroFull(payments)}: negoziare laddove ROI < target.`)
    return items
  ...
  return (
    <div className="card card-global" style={{ padding: 14 }}>
      ...
      >
        ...
        <span style={{ color: '#9ca3af', fontSize: 12 }}>{periodLabel}</span>
      </div>
      <div
        style={{
          display: 'grid',
