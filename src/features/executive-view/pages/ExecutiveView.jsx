...
export default function ExecutiveView() {
  ...
  const model = useMemo(
    () =>
      buildMonthlySeries({ mediaRows, payments, assumptions, scenarioKey: scenario, projectPlan }),
    [mediaRows, payments, assumptions, scenario, projectPlan]
  )
  ...
  const personnelSummary = useMemo(() => {
    ...
    return {
      total,
      breakdown: Array.from(breakdownMap.values()).sort((a, b) => b.monthlyCost - a.monthlyCost),
    }
  ...
  const calcRoi = (monthsArr) => {
    ...
    const totalCost = (monthsArr || []).reduce(
      (acc, m) => acc + ((m?.payouts || 0) + (m?.opex || 0)),
      0
    )
  ...
  const clampSeries = (arr, cap = 1_000_000_000) =>
    (arr || []).map((v) => {
      const n = Number(v)
  ...
  const sanitizedSeries = useMemo(
    () => ({
      ...
      margin: clampSeries(
        periodSeries.revenue.map((rev, idx) => (rev ? (periodSeries.ebitda[idx] / rev) * 100 : 0)),
        1000
      ).slice(0, maxLen),
    }),
  ...
  const assumptionNotes = {
    regGrowth:
      'Reg growth / month (%): aumenta/diminuisce il tasso di crescita mensile delle registrazioni 2026 → più registrazioni → più FTD/QFTD → più revenue e quindi più EBITDA (al netto di payouts/OPEX).',
    regToFtd:
      'Reg → FTD lift (x): moltiplica il rapporto FTD/Registrazioni nelle proiezioni 2026 → ogni punto aumenta i FTD generati per lo stesso numero di registrazioni → più revenue e più payouts; EBITDA cresce solo se la marginalità resta positiva.',
    ftdToQftd:
      'FTD → QFTD lift (x): aumenta la conversione da FTD a QFTD nelle proiezioni → più QFTD → maggiore P&L (Closed P&L) e maggiore payout/CPA dove applicato.',
    netPerFtd:
      'Net / FTD uplift (x): moltiplica il net per FTD medio → impatta direttamente la revenue P&L per ogni FTD/QFTD → EBITDA sale o scende in proporzione, payout invariato salvo effetti indiretti.',
    cpaPerQftd:
      'CPA / QFTD uplift (x): moltiplica il costo per QFTD (commissioni/CPA) → aumenta i payouts/commissioni per ogni QFTD → EBITDA si riduce se il costo cresce più del ricavo incrementale.',
    seasonality:
      'Seasonality strength: varia l’ampiezza della stagionalità applicata al profilo mensile 2026 (basato sulla forma 2025) → ridistribuisce volumi e ricavi sui mesi/quarter; il totale annuo resta guidato dagli altri uplift, ma i picchi/valle cambiano e con essi EBITDA per periodo.',
    dubaiMonthly:
      'Dubai OPEX / month (€) + start month: aggiunge un costo opex ricorrente a partire dal mese scelto nel 2026 → aumenta l’OPEX mensile/quarter e riduce l’EBITDA a partire da quel punto.',
    dubaiStart:
      'Dubai OPEX / month (€) + start month: aggiunge un costo opex ricorrente a partire dal mese scelto nel 2026 → aumenta l’OPEX mensile/quarter e riduce l’EBITDA a partire da quel punto.',
    techPlatform:
      'OPEX tech/platform, tooling, compliance (€): costi fissi mensili ricorrenti 2026 → aumentano OPEX lineare e riducono EBITDA in modo proporzionale.',
    monitoring:
      'OPEX tech/platform, tooling, compliance (€): costi fissi mensili ricorrenti 2026 → aumentano OPEX lineare e riducono EBITDA in modo proporzionale.',
    compliance:
      'OPEX tech/platform, tooling, compliance (€): costi fissi mensili ricorrenti 2026 → aumentano OPEX lineare e riducono EBITDA in modo proporzionale.',
  ...
  return (
    <div className="w-full space-y-4">
      ...
      >
        <div>
          <div style={{ fontSize: 12, color: '#9ca3af', fontWeight: 700, letterSpacing: 0.18 }}>
            Budget & Projections 2026
          </div>
          <h2 style={{ margin: 0 }}>2025 actuals → 2026 projected scenarios</h2>
          <p style={{ margin: '4px 0 0 0', color: '#9ca3af', fontSize: 13 }}>
            Board-ready view with Financials, People & OPEX, and Insights tabs.
          </p>
          <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
            {[
              { key: 'financials', label: 'Financials' },
              { key: 'people', label: 'People & OPEX' },
      ...
      {activeTab === 'financials' && (
        <>
          <div
            className="card"
            style={{
              background: 'linear-gradient(120deg, rgba(15,118,110,0.2), rgba(14,165,233,0.12))',
          ...
          >
                ...
                >
                  ...
                  >
                    ...
                    <span
                      style={{
                        color:
                          kpi.delta === null ? '#cbd5e1' : kpi.delta >= 0 ? '#34d399' : '#f97316',
                      }}
                    >
                      {kpi.delta === null ? '—' : formatPercent(kpi.delta, 1)}
                    </span>
                  </div>
                </div>
              ))}
            ...
            <div style={{ marginTop: 8, fontSize: 12, color: '#cbd5e1' }}>
              YoY deltas show 2026 vs locked 2025; ROI = P&L / (payouts + OPEX).
            </div>
          </div>
          ...
          >
            <div
              className="card"
              style={{ background: '#0b1524', border: '1px solid rgba(255,255,255,0.08)' }}
            >
              <div
                style={{
                  display: 'flex',
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ fontSize: 12, color: '#9ca3af' }}>2025 solid · 2026 dashed</div>
                </div>
                <div
                  style={{
                    display: 'flex',
                ...
                {hasVisibleData ? (
                  <PnLTrendChart
                    ...
                    series={(() => {
                      ...
                      const { actual: ebitdaActual, forecast: ebitdaForecast } = splitBarData(
                        sanitizedSeries.ebitda
                      )
                      const { actual: opexActual, forecast: opexForecast } = splitBarData(
                        sanitizedSeries.opex
                      )
                      ...
                      if (metricToggles.revenue)
                        datasets.push(
                          makeLine(sanitizedSeries.revenue, '#22d3ee', 'Closed P&L', 'y')
                        )
                      if (metricToggles.payouts)
                        datasets.push(
                          makeLine(sanitizedSeries.payouts, '#facc15', 'Commissions', 'y')
                        )
                      if (metricToggles.roi)
                        datasets.push(makeLine(sanitizedSeries.roi, '#a3e635', 'ROI', 'y1'))
                  ...
                  <div style={{ color: '#9ca3af', fontSize: 13, padding: 12 }}>
                    Nessun dato valido da mostrare (revenues/opex/payouts tutti zero o non
                    definiti).
                  </div>
                )}
              </div>
              ...
              >
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      step="0.005"
                      value={assumptions.scenarios[scenario].regGrowth}
                      onChange={(e) => handleScenarioInput('regGrowth', Number(e.target.value))}
                      className="input-num"
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 48, textAlign: 'right' }}
                    >
                      {(assumptions.scenarios[scenario].regGrowth * 100).toFixed(2)}%
                    </span>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      step="0.01"
                      value={assumptions.scenarios[scenario].regToFtdLift}
                      onChange={(e) => handleScenarioInput('regToFtdLift', Number(e.target.value))}
                      className="input-num"
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 48, textAlign: 'right' }}
                    >
                      {assumptions.scenarios[scenario].regToFtdLift.toFixed(2)}x
                    </span>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      step="0.01"
                      value={assumptions.scenarios[scenario].ftdToQftdLift}
                      onChange={(e) => handleScenarioInput('ftdToQftdLift', Number(e.target.value))}
                      className="input-num"
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 48, textAlign: 'right' }}
                    >
                      {assumptions.scenarios[scenario].ftdToQftdLift.toFixed(2)}x
                    </span>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      step="0.01"
                      value={assumptions.scenarios[scenario].netPerFtdLift}
                      onChange={(e) => handleScenarioInput('netPerFtdLift', Number(e.target.value))}
                      className="input-num"
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 48, textAlign: 'right' }}
                    >
                      {assumptions.scenarios[scenario].netPerFtdLift.toFixed(2)}x
                    </span>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      step="0.01"
                      value={assumptions.scenarios[scenario].cpaPerQftdLift}
                      onChange={(e) =>
                        handleScenarioInput('cpaPerQftdLift', Number(e.target.value))
                      }
                      className="input-num"
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 48, textAlign: 'right' }}
                    >
                      {assumptions.scenarios[scenario].cpaPerQftdLift.toFixed(2)}x
                    </span>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <input
                    ...
                    max="0.4"
                    step="0.01"
                    value={assumptions.seasonalityStrength ?? 0.15}
                    onChange={(e) =>
                      setAssumptions((prev) => ({
                        ...prev,
                        seasonalityStrength: Number(e.target.value),
                      }))
                    }
                  />
                  <span style={{ fontSize: 12, color: '#cbd5e1' }}>
                    {(assumptions.seasonalityStrength ?? 0.15).toFixed(2)} (light bias)
                  </span>
                </label>
                <label
                  style={{
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      step="1000"
                      value={assumptions.opex?.dubai?.monthly || 0}
                      onChange={(e) => handleOpexInput('dubai', 'monthly', Number(e.target.value))}
                      className="input-num"
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 72, textAlign: 'right' }}
                    >
                      {formatEuro(assumptions.opex?.dubai?.monthly || 0)}
                    </span>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <select
                    value={assumptions.opex?.dubai?.startMonth || '2026-00'}
                    onChange={(e) => handleOpexInput('dubai', 'startMonth', e.target.value)}
                    style={{
                      padding: 8,
                      borderRadius: 6,
                      background: '#111827',
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      className="input-num"
                      value={assumptions.opex.tech.hosting}
                      onChange={(e) => handleOpexInput('tech', 'hosting', Number(e.target.value))}
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 72, textAlign: 'right' }}
                    >
                      {formatEuro(assumptions.opex.tech.hosting)}
                    </span>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      className="input-num"
                      value={assumptions.opex.tech.tooling}
                      onChange={(e) => handleOpexInput('tech', 'tooling', Number(e.target.value))}
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 72, textAlign: 'right' }}
                    >
                      {formatEuro(assumptions.opex.tech.tooling)}
                    </span>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <input
                      type="number"
                      className="input-num"
                      value={assumptions.opex.legal.compliance}
                      onChange={(e) =>
                        handleOpexInput('legal', 'compliance', Number(e.target.value))
                      }
                    />
                    <span
                      style={{ fontSize: 12, color: '#cbd5e1', minWidth: 72, textAlign: 'right' }}
                    >
                      {formatEuro(assumptions.opex.legal.compliance)}
                    </span>
                  </div>
                </label>
              </div>
              <div style={{ marginTop: 8, fontSize: 12, color: '#9ca3af' }}>
                Controls steer 2026 only; 2025 remains locked.
              </div>
            </div>
          </div>
          ...
          >
            ...
            <button className="tab" onClick={() => setShowLowerCharts((v) => !v)}>
              {showLowerCharts ? 'Hide' : 'Show'}
            </button>
          </div>
          ...
          {showLowerCharts && (
            ...
            >
                    ...
                    series={[
                      {
                        ...
                        segment: {
                          borderDash: (ctx) =>
                            periodSeries.actualCount > 0 &&
                            Number(ctx?.p0DataIndex ?? -1) >= periodSeries.actualCount - 1
                              ? [6, 4]
                              : undefined,
                        },
                      },
              ...
              <div className="card">
                <div style={{ fontSize: 12, color: '#9ca3af', fontWeight: 700 }}>ROI %</div>
                <div className="h-64" style={{ marginTop: 6 }}>
                  <PnLTrendChart
                    labels={periodSeries.labels}
                    series={[
              ...
              <div className="card">
                <div style={{ fontSize: 12, color: '#9ca3af', fontWeight: 700 }}>Margin %</div>
                <div className="h-64" style={{ marginTop: 6 }}>
                  <PnLTrendChart
                    labels={periodSeries.labels}
                    series={[
          ...
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button className="tab" onClick={() => setShowDetails((v) => !v)}>
                {showDetails ? 'Hide' : 'Show'} table
              </button>
            </div>
            {showDetails && (
                  ...
                  <tbody>
                    {periodSeries.tableRows.map((row, idx) => (
                      <tr key={row.label || row.key || idx}>
                        ...
                        <td>
                          {row.registrations
                            ? (((row.ftd || 0) / Math.max(row.registrations, 1)) * 100).toFixed(1) +
                              '%'
                            : '—'}
                        </td>
                        <td>
                          {row.ftd
                            ? (((row.qftd || 0) / Math.max(row.ftd, 1)) * 100).toFixed(1) + '%'
                            : '—'}
                        </td>
                        ...
                        <td>
                          {row.opex || row.payouts
                            ? (
                                ((row.pnl ?? row.ebitda ?? 0) /
                                  Math.max((row.payouts || 0) + (row.opex || 0), 1)) *
                                100
                              ).toFixed(1) + '%'
                            : '0%'}
                        </td>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button className="tab" onClick={() => setShowAssumptions((v) => !v)}>
            ...
            {showAssumptions && (
              ...
              >
                <div className="card" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  ...
                  <ul
                    style={{ margin: '6px 0 0 12px', padding: 0, color: '#cbd5e1', fontSize: 13 }}
                  >
                    <li>
                      Reg growth: {(assumptions.scenarios[scenario].regGrowth * 100).toFixed(2)}%
                      monthly
                    </li>
                    <li>
                      Reg→FTD lift: {assumptions.scenarios[scenario].regToFtdLift.toFixed(2)}x
                    </li>
                    <li>
                      FTD→QFTD lift: {assumptions.scenarios[scenario].ftdToQftdLift.toFixed(2)}x
                    </li>
                    <li>
                      Net/FTD uplift: {assumptions.scenarios[scenario].netPerFtdLift.toFixed(2)}x
                    </li>
                    <li>
                      CPA/QFTD uplift: {assumptions.scenarios[scenario].cpaPerQftdLift.toFixed(2)}x
                    </li>
                  </ul>
                </div>
                <div className="card" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  ...
                  >
                    <li>
                      Strength: {(assumptions.seasonalityStrength ?? 0.15).toFixed(2)} (clamped
                      index 0.7–1.3)
                    </li>
                ...
                <div className="card" style={{ background: 'rgba(255,255,255,0.02)' }}>
                  ...
                  >
                    {roadmapPreview.map((p) => (
                      <li key={p.id}>
                        {p.enabled ? 'Enabled' : 'Disabled'} · {p.label} · {p.startMonth} ·{' '}
                        {p.duration}m ·{' '}
                        {formatEuro(
                          (p.costs.personnel || 0) +
                            (p.costs.tooling || 0) +
        ...
        >
          <div className="card">
            ...
            >
              <div>
                <div style={{ fontSize: 12, color: '#9ca3af', fontWeight: 700 }}>
                  People cost (org chart derived)
                </div>
                <div style={{ fontSize: 12, color: '#cbd5e1' }}>
                  Dept-first view; toggle quarter/month; expand to see roles.
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                <button
                  className={`tab ${peopleViewMode === 'quarter' ? 'active' : ''}`}
                  onClick={() => setPeopleViewMode('quarter')}
                >
                  Quarter
                </button>
                <button
                  className={`tab ${peopleViewMode === 'month' ? 'active' : ''}`}
                  onClick={() => setPeopleViewMode('month')}
                >
                  Month
                </button>
                <div className="kpi" style={{ background: 'rgba(255,255,255,0.05)' }}>
                  ...
                  <strong>
                    {peopleViewMode === 'quarter'
                      ? formatEuro(personnelSummary.total * 3)
                      : formatEuro(personnelSummary.total)}
                  </strong>
                </div>
              </div>
            </div>
            ...
            >
              ...
              />
              <button className="tab" onClick={() => setExpandedDepts({})}>
                Collapse all
              </button>
            </div>
                ...
                <tbody>
                  {(() => {
                    ...
                    const filtered = personnelRows
                      ...
                      .map((row) => ({
                        ...row,
                        salary: Number(personnelOverrides[row.id] ?? row.baseSalary ?? 0),
                      }))
                    ...
                    const totalMonthly =
                      Array.from(deptMap.values()).reduce((acc, d) => acc + d.monthly, 0) || 1
                              ...
                              <tr key={m.id} style={{ background: 'rgba(255,255,255,0.02)' }}>
                                ...
                                <td>
                                  <input
                                    type="number"
                                    className="input-num"
                                    value={personnelOverrides[m.id] ?? m.baseSalary}
                                    onChange={(e) =>
                                      handleSalaryChange(m.id, Number(e.target.value))
                                    }
                                  />
                                </td>
                                <td colSpan={2}>
                                  {formatEuro(
                                    (personnelOverrides[m.id] ?? m.baseSalary) * viewMultiplier
                                  )}
                                </td>
                              </tr>
                            ))}
          ...
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 12, color: '#9ca3af' }}>
                Costs flow into OPEX (personnel/tooling/marketing)
              </span>
            </div>
            <div className="table-wrap" style={{ marginTop: 8 }}>
              <table className="table" style={{ minWidth: 520 }}>
                    ...
                    <tr key={p.id}>
                      ...
                      <td>
                        <select
                          value={p.startMonth}
                          onChange={(e) =>
                            handleProjectChange(p.id, { startMonth: e.target.value })
                          }
                          style={{
                            padding: 6,
                            borderRadius: 6,
                      ...
                      <td>
                        <input
                          ...
                          value={p.duration}
                          onChange={(e) =>
                            handleProjectChange(p.id, { duration: Number(e.target.value) })
                          }
                        />
                      </td>
                      <td>
                        ...
                        >
                          <input
                            type="number"
                            className="input-num"
                            value={p.costs.personnel}
                            onChange={(e) =>
                              handleProjectChange(p.id, {
                                costs: { personnel: Number(e.target.value) },
                              })
                            }
                          />
                          <input
                            type="number"
                            className="input-num"
                            value={p.costs.tooling}
                            onChange={(e) =>
                              handleProjectChange(p.id, {
                                costs: { tooling: Number(e.target.value) },
                              })
                            }
                          />
                          <input
                            type="number"
                            className="input-num"
                            value={p.costs.marketing}
                            onChange={(e) =>
                              handleProjectChange(p.id, {
                                costs: { marketing: Number(e.target.value) },
                              })
                            }
                          />
                        </div>
                      ...
                      <td>
                        <button
                          className={`tab ${p.enabled ? 'active' : ''}`}
                          onClick={() => handleProjectChange(p.id, { enabled: !p.enabled })}
                        >
            ...
            <div style={{ marginTop: 6, fontSize: 12, color: '#9ca3af' }}>
              Impact reflected month-by-month when window is active.
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'insights' && (
        ...
        >
          ...
          >
            <div>
              <div style={{ fontSize: 12, color: '#9ca3af', fontWeight: 700 }}>Insights</div>
              <div style={{ fontSize: 12, color: '#cbd5e1' }}>
                Board-ready highlights derived from the current scenario.
              </div>
            </div>
            <span style={{ fontSize: 12, color: '#9ca3af' }}>
              2025 vs 2026 diffs auto-updated per scenario
            </span>
          </div>
          <div
            style={{
          ...
          >
            <div className="card" style={{ background: 'rgba(255,255,255,0.02)' }}>
              ...
              <ul style={{ margin: '6px 0 0 12px', padding: 0, color: '#cbd5e1', fontSize: 13 }}>
                <li>
                  Revenue YoY:{' '}
                  {formatPercent(yoyDelta(totals2026.revenue, totals2025.revenue) || 0, 1)}
                </li>
                <li>
                  EBITDA YoY:{' '}
                  {formatPercent(yoyDelta(totals2026.ebitda, totals2025.ebitda) || 0, 1)}
                </li>
                <li>
                  ROI avg: {totals2026.roi.toFixed(1)}% (2025: {totals2025.roi.toFixed(1)}%)
                </li>
              </ul>
            </div>
            <div className="card" style={{ background: 'rgba(255,255,255,0.02)' }}>
              ...
              <ul style={{ margin: '6px 0 0 12px', padding: 0, color: '#cbd5e1', fontSize: 13 }}>
                <li>
                  OPEX 2026: {formatEuro(totals2026.opex)} (vs {formatEuro(totals2025.opex)})
                </li>
                <li>
                  Payouts mix: {formatEuro(totals2026.payouts)} vs revenue{' '}
                  {formatEuro(totals2026.revenue)}
                </li>
            ...
            <div className="card" style={{ background: 'rgba(255,255,255,0.02)' }}>
              ...
              <ul style={{ margin: '6px 0 0 12px', padding: 0, color: '#cbd5e1', fontSize: 13 }}>
                ...
                <li>
                  QFTD YoY: {formatPercent(yoyDelta(totals2026.qftd, totals2025.qftd) || 0, 1)}
                </li>
                <li>
                  Seasonality strength: {(assumptions.seasonalityStrength ?? 0.15).toFixed(2)}
                </li>
              </ul>
            </div>
          </div>
