...
export default function TemplateKpiTable({ livePayload }) {
  ...
  function performanceBadge(replyRate, sent) {
    ...
    if (replyRate >= 25) return { label: 'Ottimo', cls: 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/30' }
    if (replyRate >= 15) return { label: 'Buono', cls: 'bg-sky-500/15 text-sky-300 border border-sky-500/30' }
    if (replyRate >= 7) return { label: 'Da migliorare', cls: 'bg-amber-500/15 text-amber-300 border border-amber-500/30' }
  ...
  if (!configured) {
    return (
      <section className="rounded-2xl border border-slate-800 bg-slate-900/75 p-6">
        <h2 className="text-sm font-semibold text-slate-200">Performance template</h2>
        <div className="mt-4 rounded-xl border border-amber-400/20 bg-amber-500/8 p-4 text-xs text-amber-100">
          ...
          <p className="mt-1 opacity-85">Imposta <code className="text-amber-200">CONVRS_API_TOKEN</code> nel file <code className="text-amber-200">.env.server.local</code> e riavvia il server.</p>
        </div>
      </section>
    )
  }
          ...
          {tracking ? (
            <>
              ...
              <p className="mt-1">
                ...
                {' · '}Attribuiti a template: <span className="text-slate-300">{tracking.attributedMessages}</span>
              </p>
            </>
          ) : (
            <p>Nessun dato disponibile. Invia il primo template e registra il mid con <code>/api/convrs/register-send</code>.</p>
          )}
        </div>
        <div className="mt-4 rounded-xl border border-slate-700 bg-slate-800/50 p-4 text-xs text-slate-300">
          <p className="font-semibold text-slate-200 mb-2">Come far apparire i dati</p>
          <ol className="list-decimal list-inside space-y-1 text-slate-400">
            <li>Invia un template WhatsApp via API Convrs (salva il <code className="text-slate-300">mid</code> restituito)</li>
            <li>
              Chiama{' '}
            ...
            <li>Convrs invierà i callback ACK al webhook — i KPI vengono aggiornati automaticamente</li>
          </ol>
        </div>
      </section>
    )
  ...
  return (
    <section className="rounded-2xl border border-slate-800 bg-slate-900/75 overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-y border-slate-800 bg-slate-950/50 text-left text-slate-500 uppercase tracking-widest">
            ...
            {templateList.map((row) => {
              ...
              return (
                <tr key={row.key} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-3 py-3 text-right text-slate-300">{fmt(row.sent)}</td>
                  <td className="px-3 py-3 text-right text-slate-300">{fmt(row.delivered)}</td>
                  <td className="px-3 py-3 text-right text-slate-400">{fmtRate(row.deliveryRate)}</td>
                  <td className="px-3 py-3 text-right text-slate-300">{fmt(row.read)}</td>
                  <td className="px-3 py-3 text-right text-slate-400">{fmtRate(row.readRate)}</td>
                  <td className="px-3 py-3 text-right text-slate-300">{fmt(row.replies)}</td>
                  <td className="px-3 py-3 text-right font-semibold text-slate-200">{fmtRate(row.replyRate)}</td>
                  <td className="px-3 py-3 text-right text-red-400/70">{row.failed > 0 ? fmt(row.failed) : '—'}</td>
      ...
      <p className="px-5 py-2.5 text-[10px] text-slate-600 border-t border-slate-800">
        Dati basati su callback Convrs e messaggi tracciati per mid. Nessun dato storico disponibile via API pubblica.
      </p>
    </section>
  )
}
