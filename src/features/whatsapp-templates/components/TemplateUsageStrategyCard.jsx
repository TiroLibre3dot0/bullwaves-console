export default function TemplateUsageStrategyCard({ template }) {
  ...
  return (
    <section className="rounded-xl border border-gray-600 bg-gray-700/70 p-4">
      <h3 className="text-sm font-semibold text-gray-200">Usage strategy</h3>
      <div className="mt-3 space-y-3">
        {rows.map((row) => (
          <div key={row.label} className="rounded-lg border border-gray-600 bg-gray-900/60 p-3">
            <p className="text-xs uppercase tracking-[0.08em] text-slate-500">{row.label}</p>
            <p className="mt-1 text-sm text-gray-200">{row.value}</p>
          </div>
        ))}
      </div>
    </section>
