export default function WhatsAppTemplatesLiveStatus({ liveState, onManualSync, syncing }) {
  ...
  return (
    <section className="rounded-2xl border border-gray-600 bg-gray-700/75 p-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-200">
            ...
            {' · '}Persistence: <span className="text-gray-300">{persistence}</span>
          </p>
        ...
        <button
          ...
          className="rounded-lg border border-gray-600 bg-gray-900/70 px-3 py-2 text-xs font-semibold text-gray-200 transition hover:border-sky-400/50 hover:text-sky-200 disabled:cursor-not-allowed disabled:opacity-50"
        >
      ...
      <div className="mt-3 rounded-xl border border-amber-400/20 bg-amber-500/8 p-3 text-xs text-amber-100">
        ...
        <p className="mt-1 text-amber-100/85">
          Historical template analytics, sent-template listings and template catalogs are not
          exposed via the public API. This dashboard can only build WhatsApp KPIs from callbacks and
          from message details for known mids.
        </p>
