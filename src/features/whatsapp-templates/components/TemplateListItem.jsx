  ...
  return (
    <button
      ...
      className={`w-full rounded-xl border p-4 text-left transition ${
        selected
          ? 'border-sky-400/70 bg-linear-to-br from-slate-800/95 to-slate-900 shadow-lg shadow-sky-500/10'
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <span className="rounded-full border border-slate-600 bg-slate-800/80 px-2 py-0.5 text-[11px] font-semibold text-slate-200">
          {template.tone}
        </span>
      </div>

      <p className="mt-3 line-clamp-2 text-xs leading-relaxed text-slate-300">{template.snippet}</p>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-2">
          ...
          <div className="font-semibold text-slate-200">{formatPercent(template.stats.replyRate)}</div>
        </div>
        <div className="rounded-lg border border-slate-800 bg-slate-950/60 p-2">
