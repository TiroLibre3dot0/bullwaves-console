...
### Conferma Supporto (Acuity)

- Causa radice confermata dal supporto Acuity: per l'account Bullwaves, Market Insight / Acuity News / Acuity Events sono abilitati solo in inglese.
- LanguageCode funzionante per questi endpoint: en-gb
- LanguageCode non funzionante per questi endpoint: it-it (ritorna 403)

### Policy Lingua Applicata (Bullwaves)

- en-gb applicato agli endpoint che accettano LanguageCode nel nostro flusso:
  - POST /api/signalcentrereports
  - POST /api/signalcentre/products
  - POST /api/marketinsights
  - POST /api/AcuityNews
  - POST /api/AcuityEvents
- Nessun campo LanguageCode usato per:
  - POST /api/percentagepositivity/multiassets
  - POST /api/sentiment/value/multiassets
  - modello validato: {"AssetIds":[50],"SentimentTypeId":12,"Period":1}

### Knowledge Base - API Principali
...
### Endpoint REST da Testare/Monitorare
...
- Funzionanti su account corrente (osservato):
  ...
  - /api/marketinsights (200 con en-gb)
  - /api/AcuityNews (200 con en-gb)
  - /api/AcuityEvents (200 con en-gb)

- Bloccati su account corrente (osservato):
  - /api/fxevents (403 Forbidden)
  - /api/fxnews (403 Forbidden) - atteso in base alla discussione sulla subscription

- Modelli request validati (attuali):
  ...
  - /api/fxevents
    - {"StartDate":"2026-05-01","EndDate":"2026-05-31","LanguageCode":"it-it","Count":5} -> 403 (non abilitato)

- Modelli request aggiornati dopo conferma supporto:
  - /api/marketinsights
    - {"StartDate":"2026-05-01","EndDate":"2026-05-04","LanguageCode":"en-gb","ArticleTypes":["macroeconomicAsset"],"Count":5}
  - /api/AcuityNews
    - {"StartDate":"2026-05-01","EndDate":"2026-05-04","LanguageCode":"en-gb","Count":5}
...
- Verifica casing:
  - /api/AcuityNews e /api/acuitynews restituiscono entrambi 403 (stesso risultato)
  - /api/AcuityEvents e /api/acuityevents restituiscono entrambi 200 (stesso risultato)

## English

### Support Confirmation (Acuity)

- Root cause confirmed by Acuity support: for Bullwaves account, Market Insight / Acuity News / Acuity Events are enabled for English only.
- Working LanguageCode for these endpoints: en-gb
- Failing LanguageCode for these endpoints: it-it (returns 403)
